...
// server middleware
/* separate post methods are necessary for each endpoint for slug to work */
server.post("/tasks/", function(req, res, next) {
  ...
  if (error) {
    ...
    req.body.slug = createSlug(req.body.title); // Generate a slug for new posts
    next();
  }
});
...
server.post("/categories/", function(req, res, next) {
  ...
  if (error) {
    ...
    req.body.slug = createSlug(req.body.name); // Generate a slug for new posts
    next();
  }
});

server.post("/checklists/", function(req, res, next) {
...
/* I don't think you can do server-side validation with the create-reacta-app
...
function validateChecklist(checklist) {
  // if (!checklist.name) return "Name is required.";
  if (!checklist.title) return "Title is required.";
  if (checklist.tasks.length < 2) return "Must have at least 2 tasks.";
  return "";
}
